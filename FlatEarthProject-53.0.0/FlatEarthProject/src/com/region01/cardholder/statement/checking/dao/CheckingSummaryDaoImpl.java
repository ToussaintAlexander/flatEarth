...
public class CheckingSummaryDaoImpl implements CheckingSummaryDao {
...
public int insert(CheckingSummaryBean credit){
			...
			String sql 
				...
				= "INSERT INTO CHECKING_SUMMARY_TABLE "
						+ "("
...
public int delete(int id){
	...
	String sql =
			...
			" DELETE FROM CHECKING_SUMMARY_TABLE " 	+
			"  WHERE ID = "							+
			         id								+
			"";
...
public CheckingSummaryBean selectCheckingSummaryById(int id){
	...
	String sql 
		...
		= "SELECT * "
		+ "  FROM CHECKING_SUMMARY_TABLE "
		+ " WHERE ID = ?";
	
	return template.queryForObject( sql
				      , new Object[]{id}
				      , new BeanPropertyRowMapper<CheckingSummaryBean>(CheckingSummaryBean.class));
...
public List<CheckingSummaryBean> select(){
	...
	return template.query( "SELECT * FROM CHECKING_SUMMARY_TABLE WHERE ID < 11"
			     ,new RowMapper<CheckingSummaryBean>(){
		
		public CheckingSummaryBean mapRow(ResultSet rs, int row) throws SQLException {
			
			CheckingSummaryBean credit = new CheckingSummaryBean();
