...
public class SavingsChargeDaoImpl implements SavingsChargeDao {
...
public int insert(SavingsChargeBean charge){
			...
			String sql 
				...
				= "INSERT INTO SAVINGS_CHARGE_TABLE "
						+ "("
...
public SavingsChargeBean selectSavingsChargeById(int id){
	...
	String sql 
		...
		= "SELECT * "
		+ "  FROM SAVINGS_CHARGE_TABLE "
		+ " WHERE ID = ?";
	
	return template.queryForObject( sql
				      , new Object[]{id}
				      , new BeanPropertyRowMapper<SavingsChargeBean>(SavingsChargeBean.class));
...
public List<SavingsChargeBean> select(){
	...
	return template.query( "SELECT * FROM SAVINGS_CHARGE_TABLE WHERE ID < 11"
			     ,new RowMapper<SavingsChargeBean>(){
		
		public SavingsChargeBean mapRow(ResultSet rs, int row) throws SQLException {
			
			SavingsChargeBean charge = new SavingsChargeBean();
